...
z3 = a2 * Theta2'
hyp = sigmoid(z3)
Theta1_2 = Theta1(:,2:end)
Theta2_2 = Theta2(:,2:end)
Reg = (lambda/(2 *m)) * (sum(sum(Theta1_2 .^2)) + sum(sum(Theta2_2 .^ 2)))
tmp = eye(num_labels);
yk  = (tmp(y,:));
J = (1/m) * sum(sum(-yk .* log(hyp) - (1-yk) .* log(1-hyp)))
J = (1/m) * sum(sum(-yk .* log(hyp) - (1-yk) .* log(1-hyp))) + Reg
...
DELTA_1_2 = DELTA_1(:,2:end)
DELTA_2_2 = DELTA_2(:,2:end)
